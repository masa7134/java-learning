...
public class Ex1_14_2 {
	...
	*/
	static int calcTaxPayment( int discountedPrice ){        //アンダーバーを適切な内容に書き換えてください
		int calcResult = 0;
		calcResult = (int)(discountedPrice * 1.08);
		return calcResult;
	}
	...
	*/
	static int discount( int totalPrice ){//アンダーバーを適切な内容に書き換えてください
		...
		int discountPrice = 0; // 値引きする金額 ここで必ずint型をしようすること　なぜなら小数点の円は存在しないから！！！
		if(totalPrice >= DISCOUNT_BORDER1 && totalPrice <= DISCOUNT_BORDER2) {
			...
			discountPrice = discountPrice + (int)((totalPrice - DISCOUNT_BORDER2) * DISCOUNT_RATE2);
		} // else 必要なし

		if(discountPrice >= DISCOUNT_PRICE_MAX){
			totalPrice = totalPrice - DISCOUNT_PRICE_MAX;
		}else {
